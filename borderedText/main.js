...
content.forEach((value) => {
	...
	ts.setAttributeNS(null, 'dy', '1.4em');
...
for (let i = 0; i < boxes.length - 1; i++) {
	...
	boxes[i].height += 1;
	boxes[i + 1].height += boxes[i + 1].y - boxes[i].y - boxes[i].height - 3;
...
for (let i = 0; i < boxes.length; i++) {
	let m1 =
		Math.min(
			Math.abs(boxes[i].width - (boxes[i - 1] === undefined ? 0 : boxes[i - 1].width)) /
				2,
			boxes[i].height
		) / 2;
	...
	let m2 =
		Math.min(
			Math.abs((boxes[i + 1] === undefined ? 0 : boxes[i + 1].width) - boxes[i].width) /
				2,
			boxes[i].height
		) / 2;
	...
	d += `L 
    ${boxes[i].x + boxes[i].width + (boxes[i].width > (boxes[i - 1] === undefined ? 0 : boxes[i - 1].width) ? -m1 : m1)} 
    ${boxes[i].y}
    ...
    ${boxes[i].x + boxes[i].width + (boxes[i].width > (boxes[i + 1] === undefined ? 0 : boxes[i + 1].width) ? -m2 : m2)} 
    ${boxes[i].y + boxes[i].height}`;
}
...
for (let i = boxes.length - 1; i >= 0; i--) {
    ...
    d += `L
    ${boxes[i].x + (boxes[i].width < (boxes[i + 1] === undefined ? 0 : boxes[i + 1].width) ? -m1 : m1)}
    ${boxes[i].y + boxes[i].height}
    ...
    A ${m2} ${m2} 0 0 ${+(boxes[i].width > (boxes[i - 1] === undefined ? 0 : boxes[i - 1].width))}
    ${boxes[i].x + (boxes[i].width > (boxes[i - 1] === undefined ? 0 : boxes[i - 1].width) ? m2 : -m2)}
    ${boxes[i].y}
    `;
}
